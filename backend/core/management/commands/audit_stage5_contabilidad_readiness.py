...
def _validate_output_path(output_path: Path) -> None:
    ...
    try:
        ...
        raise CommandError(
            ...
            'para no versionar evidencia o metadatos contables.'
        ) from error
    ...
    def handle(self, *args, **options):
        ...
        try:
            result = collect_stage5_contabilidad_readiness(
                stage3_evidence_ref=options['stage3_evidence_ref'],
            ...
            raise CommandError(
                'No se pudo auditar readiness Etapa 5 porque la base configurada no esta migrada o no es accesible.'
            ) from error
        ...
        if options['fail_on_attention'] and not result['ready_for_stage5_contabilidad']:
            raise CommandError(
                'Readiness Etapa 5 Contabilidad no cerrada: '
