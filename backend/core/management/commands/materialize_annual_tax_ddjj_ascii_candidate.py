...
def _validate_output_dir(output_dir: Path) -> None:
    validate_local_evidence_output_dir_path(
        output_dir,
        artifact_description='archivos DDJJ ni identificadores tributarios',
    )
...
class Command(BaseCommand):
    help = (
        'Materializa y verifica un candidato DDJJ ASCII local desde un AnnualTaxExport preparado '
        'y registros revisados; no imprime RUT ni registros crudos.'
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--records-json', default='', help='Lista JSON de registros DDJJ revisados.')
        parser.add_argument('--records-file', default='', help='Archivo JSON con registros DDJJ revisados.')
        parser.add_argument(
            '--output-dir',
            default='',
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            written = write_annual_tax_ddjj_ascii_export_candidate(candidate, output_dir)
            verification = verify_annual_tax_ddjj_ascii_export_candidate(candidate, output_dir)
        except ValueError as error:
            raise CommandError(f'No se pudo materializar/verificar candidato DDJJ ASCII: {error}') from error
        ...
        result = {
            ...
            'file_extension': summary['file_extension'],
            ...
            'ddjj_record_review_evidence_total': verification['ddjj_record_review_evidence_total'],
