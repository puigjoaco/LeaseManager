...
class Command(BaseCommand):
    help = (
        'Audita el preflight de handoff responsable AC/AT: preguntas, template '
        'y review listo bajo local-evidence, sin imprimir rutas ni contenido privado.'
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action='store_true',
            help='Falla si no hay exactamente un review responsable listo para handoff.',
        )
        parser.add_argument(
            '--require-answer-template-ready',
            action='store_true',
            help='Falla si no hay exactamente un paquete de preguntas y template listos para completar respuestas.',
        )
    ...
    def handle(self, *args, **options):
        ...
        audit = audit_company_accounting_responsible_handoff_preflight(search_root=search_root)

        if options['require_review_ready'] and not audit['summary']['ready_for_responsible_decision_handoff']:
            raise CommandError('No existe exactamente un review responsable listo bajo local-evidence/.')
        if options['require_answer_template_ready'] and not audit['summary']['ready_for_responsible_answer_completion']:
            raise CommandError('No existe exactamente un paquete de preguntas y template listo bajo local-evidence/.')
        ...
        if output:
            ...
            try:
                ...
                raise CommandError('No se pudo escribir preflight de handoff responsable.') from error
