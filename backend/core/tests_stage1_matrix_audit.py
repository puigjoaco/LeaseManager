...
class Stage1MatrixAuditTests(TestCase):
    ...
    def test_existing_contract_adjustment_blank_justification_is_blocking(self):
        contrato = self._create_valid_stage1_matrix()
        AjusteContrato.objects.create(
            contrato=contrato,
            tipo_ajuste='cargo_sin_justificacion',
            monto=Decimal('1000.00'),
            moneda=MonedaBaseContrato.CLP,
            mes_inicio=date(2026, 1, 1),
            mes_fin=date(2026, 1, 1),
    ...
    def test_existing_contract_adjustment_sensitive_justification_is_blocking(self):
        ...
        adjustment = AjusteContrato.objects.create(
            contrato=contrato,
            tipo_ajuste='cargo_justificacion_sensible',
            monto=Decimal('1000.00'),
            moneda=MonedaBaseContrato.CLP,
            mes_inicio=date(2026, 1, 1),
            mes_fin=date(2026, 1, 1),
        ...
        AjusteContrato.objects.filter(pk=adjustment.pk).update(
            justificacion='Cargo respaldado en https://example.test/adjustment?token=secret'
        )
        ...
        self.assertFalse(result['ready_for_stage1_close'])
        self.assertEqual(result['classification'], 'defectuoso')
        self.assertIn('stage1.ajuste_contrato.justificacion_sensible', issue_codes)
        self.assertIn('stage1.ajuste_contrato.validacion_modelo', issue_codes)
        self.assertEqual(
            result['aggregate_classification']['ajustes_contrato']['classification'],
            'defectuoso',
    ...
    def test_early_terminated_partial_month_with_actorless_audit_event_is_blocking(self):
        contrato = self._create_valid_stage1_matrix()
        partial_contract = Contrato.objects.create(
    ...
    def test_early_terminated_partial_month_with_misaligned_audit_event_is_blocking(self):
        contrato = self._create_valid_stage1_matrix()
        partial_contract = Contrato.objects.create(
            codigo_contrato='CON-EARLY-PARTIAL-META',
            mandato_operacion=contrato.mandato_operacion,
            arrendatario=contrato.arrendatario,
            fecha_inicio=date(2025, 1, 1),
            fecha_fin_vigente=date(2025, 6, 15),
            terminacion_anticipada_prorrata_ref='early-term-proration-meta',
            terminacion_anticipada_prorrata_motivo='Prorrata aprobada por decision controlada.',
            dia_pago_mensual=5,
            estado=EstadoContrato.EARLY_TERMINATED,
        )
        ...
        AuditEvent.objects.create(
            ...
            entity_id=str(partial_contract.pk),
            summary='Decision auditada para prorrata de termino anticipado.',
            actor_identifier='stage1-early-termination-actor',
            metadata={
                'terminacion_anticipada_prorrata_ref': 'early-term-proration-meta',
    ...
    def test_early_terminated_partial_month_with_audit_actor_can_pass_stage1_matrix_gate(self):
        contrato = self._create_valid_stage1_matrix()
        partial_contract = Contrato.objects.create(
            codigo_contrato='CON-EARLY-PARTIAL-AUDIT-OK',
            mandato_operacion=contrato.mandato_operacion,
            arrendatario=contrato.arrendatario,
            fecha_inicio=date(2025, 1, 1),
            fecha_fin_vigente=date(2025, 6, 15),
            terminacion_anticipada_prorrata_ref='early-term-proration-audited-ok',
            terminacion_anticipada_prorrata_motivo='Prorrata aprobada por decision controlada.',
            dia_pago_mensual=5,
            estado=EstadoContrato.EARLY_TERMINATED,
        )
    ...
    def test_notice_sensitive_causal_is_blocking(self):
        contrato = self._create_valid_stage1_matrix()
        AvisoTermino.objects.create(
            contrato=contrato,
            fecha_efectiva=date(2026, 12, 31),
    ...
    def test_guarantee_history_sensitive_justification_is_blocking(self):
        contrato = self._create_valid_stage1_matrix()
        garantia = contrato.garantia_contractual
        garantia.monto_pactado = Decimal('100000.00')
        garantia.monto_recibido = Decimal('100000.00')
        garantia.fecha_recepcion = date(2026, 1, 5)
        garantia.estado_garantia = EstadoGarantia.HELD
        garantia.save(
            update_fields=[
        ...
        HistorialGarantia.objects.create(
            garantia_contractual=garantia,
            tipo_movimiento=TipoMovimientoGarantia.DEPOSIT,
            monto_clp=Decimal('100000.00'),
            fecha=date(2026, 1, 5),
            justificacion='Deposito respaldado en https://example.test/receipt?token=secret',
        )
