...
class Stage2CobranzaReadinessTests(TestCase):
    ...
    def test_manual_uf_without_audit_event_is_blocking(self):
        self._create_payment_matrix()
        self._create_valid_email_gate()
        self._create_valid_webpay_gate()
    ...
    def test_bootstrap_demo_manual_uf_records_audit_event(self):
        ...
        uf_value = ValorUFDiario.objects.get(fecha=date(2026, 1, 1))
    ...
    def test_residual_code_active_before_contract_end_is_blocking(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        self._create_valid_webpay_gate()
        CodigoCobroResidual.objects.create(
            ...
            saldo_actual=Decimal('25000.00'),
            estado='activa',
            fecha_activacion=date(2026, 12, 31),
        )
    ...
    def test_message_with_sensitive_block_reason_is_blocking(self):
        fixture = self._create_payment_matrix()
        email_gate = self._create_valid_email_gate()
        self._create_valid_webpay_gate()
        MensajeSaliente.objects.create(
            ...
            arrendatario=fixture['tenant'],
            destinatario=fixture['tenant'].email,
            asunto='Motivo sensible heredado',
            cuerpo='No debe exponer motivo sensible.',
            estado=EstadoMensajeSaliente.BLOCKED,
            motivo_bloqueo='Bloqueado por https://mail.example.test/callback?token=secret',
        )
        ...
        self.assertFalse(result['ready_for_stage2_cobranza'])
        self.assertIn('stage2.message.block_reason_sensitive', issue_codes)
        self.assertEqual(result['sections']['messages']['block_reason_sensitive'], 1)
        self.assertNotIn('mail.example.test', json.dumps(result))
        self.assertNotIn('callback?token=secret', json.dumps(result))
    ...
    def test_blocked_whatsapp_with_untraced_fallback_resolution_is_blocking(self):
        ...
        message = MensajeSaliente.objects.create(
            ...
            cuerpo='Cobranza controlada',
            estado=EstadoMensajeSaliente.BLOCKED,
            motivo_bloqueo='WhatsApp bloqueado sin actor trazable.',
        )
        ManualResolution.objects.create(
            category=WHATSAPP_FALLBACK_REQUIRED_CATEGORY,
            scope_type='canales',
    ...
    def test_confirmed_webpay_without_manual_audit_event_is_blocking(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        payment = fixture['payment']
        payment.estado_pago = EstadoPago.PAID
        payment.monto_pagado_clp = payment.monto_calculado_clp
        payment.fecha_pago_webpay = date(2026, 1, 6)
        payment.fecha_deteccion_sistema = date(2026, 1, 6)
        payment.save()
        IntentoPagoWebPay.objects.create(
            pago_mensual=payment,
            gate_cobro=webpay_gate,
            provider_key='transbank_webpay',
            monto_clp_snapshot=payment.monto_calculado_clp,
            buy_order='LM-PM-STAGE2-AUDIT-MISSING',
            session_id='LM-WP-STAGE2-AUDIT-MISSING',
            return_url_ref='webpay-return-controlled-v1',
            estado=EstadoIntentoPagoWebPay.CONFIRMED_MANUAL,
            external_ref='TBK-STAGE2-AUDIT-MISSING',
            fecha_pago_webpay=date(2026, 1, 6),
        )
        ...
        self.assertFalse(result['ready_for_stage2_cobranza'])
        self.assertIn('stage2.webpay_intent.confirmed_manual_event_missing', issue_codes)
        self.assertEqual(result['sections']['webpay']['confirmed_manual_event_missing'], 1)

    def test_confirmed_webpay_with_aligned_manual_audit_event_is_accepted(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        payment = fixture['payment']
        ...
        intent = IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=payment.monto_calculado_clp,
            buy_order='LM-PM-STAGE2-AUDIT-OK',
            session_id='LM-WP-STAGE2-AUDIT-OK',
            return_url_ref='webpay-return-controlled-v1',
            estado=EstadoIntentoPagoWebPay.CONFIRMED_MANUAL,
            external_ref='TBK-STAGE2-AUDIT-OK',
            fecha_pago_webpay=date(2026, 1, 6),
        )
        AuditEvent.objects.create(
        ...
        self.assertNotIn('stage2.webpay_intent.confirmed_manual_event_missing', issue_codes)

    def test_confirmed_webpay_with_misaligned_manual_audit_event_is_blocking(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        payment = fixture['payment']
        ...
        intent = IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=payment.monto_calculado_clp,
            buy_order='LM-PM-STAGE2-AUDIT-MISALIGNED',
            session_id='LM-WP-STAGE2-AUDIT-MISALIGNED',
            return_url_ref='webpay-return-controlled-v1',
            estado=EstadoIntentoPagoWebPay.CONFIRMED_MANUAL,
            external_ref='TBK-STAGE2-AUDIT-MISALIGNED',
            fecha_pago_webpay=date(2026, 1, 6),
        )
        AuditEvent.objects.create(
            event_type=WEBPAY_MANUAL_CONFIRM_EVENT_TYPE,
    ...
    def test_prepared_or_blocked_webpay_without_prepare_audit_event_is_blocking(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=fixture['payment'].monto_calculado_clp,
            buy_order='LM-PM-STAGE2-PREP-AUDIT-MISSING',
            session_id='LM-WP-STAGE2-PREP-AUDIT-MISSING',
            return_url_ref='webpay-return-controlled-v1',
        ...
        IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=fixture['payment'].monto_calculado_clp,
            buy_order='LM-PM-STAGE2-BLOCK-AUDIT-MISSING',
            session_id='LM-WP-STAGE2-BLOCK-AUDIT-MISSING',
            estado=EstadoIntentoPagoWebPay.BLOCKED,
            motivo_bloqueo='Gate WebPay bloqueado en snapshot heredado.',
        )
        ...
        self.assertFalse(result['ready_for_stage2_cobranza'])
        self.assertIn('stage2.webpay_intent.prepared_event_missing', issue_codes)
        self.assertEqual(result['sections']['webpay']['prepared_event_missing'], 2)

    def test_prepared_webpay_with_unaligned_prepare_audit_event_is_blocking(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        intent = IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=fixture['payment'].monto_calculado_clp,
            buy_order='LM-PM-STAGE2-PREP-AUDIT-BAD',
            session_id='LM-WP-STAGE2-PREP-AUDIT-BAD',
            return_url_ref='webpay-return-controlled-v1',
            estado=EstadoIntentoPagoWebPay.PREPARED,
        )
        AuditEvent.objects.create(
            ...
            metadata={
                ...
                'provider_key': 'transbank_webpay',
                'return_url_ref': 'webpay-other-return-controlled-v1',
            },
        )
        ...
        self.assertFalse(result['ready_for_stage2_cobranza'])
        self.assertIn('stage2.webpay_intent.prepared_event_missing', issue_codes)
        self.assertEqual(result['sections']['webpay']['prepared_event_missing'], 1)

    def test_prepared_webpay_with_aligned_prepare_audit_event_is_accepted(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        intent = IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=fixture['payment'].monto_calculado_clp,
            buy_order='LM-PM-STAGE2-PREP-AUDIT-OK',
            session_id='LM-WP-STAGE2-PREP-AUDIT-OK',
            return_url_ref='webpay-return-controlled-v1',
            estado=EstadoIntentoPagoWebPay.PREPARED,
        )
        AuditEvent.objects.create(
        ...
        self.assertTrue(result['ready_for_stage2_cobranza'])
        self.assertNotIn('stage2.webpay_intent.prepared_event_missing', issue_codes)
    ...
    def test_webpay_intent_with_sensitive_block_reason_is_blocking(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=fixture['payment'].monto_calculado_clp,
            buy_order='LM-PM-STAGE2-BLOCK',
            session_id='LM-WP-STAGE2-BLOCK',
            estado=EstadoIntentoPagoWebPay.BLOCKED,
            motivo_bloqueo='Bloqueo heredado en https://transbank.example.test/block?token=secret',
        )
        ...
        self.assertFalse(result['ready_for_stage2_cobranza'])
        self.assertIn('stage2.webpay_intent.sensitive_block_reason', issue_codes)
        self.assertEqual(result['sections']['webpay']['sensitive_block_reason'], 1)
        self.assertNotIn('transbank.example.test', json.dumps(result))

    def test_blocked_webpay_intent_without_reason_is_blocking(self):
        fixture = self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        IntentoPagoWebPay.objects.create(
            ...
            provider_key='transbank_webpay',
            monto_clp_snapshot=fixture['payment'].monto_calculado_clp,
            buy_order='LM-PM-STAGE2-BLOCK-MISSING',
            session_id='LM-WP-STAGE2-BLOCK-MISSING',
            estado=EstadoIntentoPagoWebPay.BLOCKED,
            motivo_bloqueo='   ',
        )
    ...
    def test_channel_gate_with_sensitive_key_is_blocking(self):
        self._create_payment_matrix()
        email_gate = self._create_valid_email_gate()
        self._create_valid_webpay_gate()
        CanalMensajeria.objects.filter(pk=email_gate.pk).update(
        ...
        self.assertFalse(result['ready_for_stage2_cobranza'])
        self.assertIn('stage2.channel_gate_sensitive_reference', issue_codes)
        self.assertNotIn('email-api-key-controlled', json.dumps(result))
    ...
    def test_webpay_gate_with_sensitive_key_is_blocking(self):
        self._create_payment_matrix()
        self._create_valid_email_gate()
        webpay_gate = self._create_valid_webpay_gate()
        GateCobroExterno.objects.filter(pk=webpay_gate.pk).update(
            restricciones_operativas={'access_token': 'webpay-access-token-controlled'}
        )
        ...
        self.assertFalse(result['ready_for_stage2_cobranza'])
        self.assertIn('stage2.webpay_gate_sensitive_reference', issue_codes)
        self.assertNotIn('webpay-access-token-controlled', json.dumps(result))
