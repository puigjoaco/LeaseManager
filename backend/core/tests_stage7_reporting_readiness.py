...
class Stage7ReportingReadinessTests(TestCase):
    ...
    def test_annual_status_updated_event_without_transition_metadata_is_blocking(self):
        ...
        self.assertIn('stage7.reporting.audit_annual_status_transition_metadata_missing', issue_codes)
    ...
    def test_annual_status_updated_event_without_review_responsible_is_blocking(self):
        AuditEvent.objects.create(
            ...
            entity_id='1',
            summary='Actualizacion anual heredada sin responsable auditado para reporting.',
            metadata={
        ...
        self.assertIn('stage7.reporting.audit_annual_status_responsible_ref_missing', issue_codes)
    ...
    def test_annual_status_updated_event_with_sensitive_review_responsible_is_blocking_without_leak(self):
        AuditEvent.objects.create(
            ...
            entity_type='f22_preparacion',
            entity_id='1',
            summary='Actualizacion anual heredada con responsable sensible para reporting.',
            metadata={
                'campo_estado': 'estado_preparacion',
                'estado_anterior': EstadoPreparacionTributaria.PREPARED,
                'estado_nuevo': EstadoPreparacionTributaria.APPROVED,
        ...
        self.assertIn('stage7.reporting.audit_annual_status_responsible_ref_sensitive', issue_codes)
    ...
    def test_authorized_source_sensitive_trace_refs_are_classified(self):
        ...
        result = collect_stage7_reporting_readiness(
            ...
            responsible_ref='stage7-reporting-responsibles-v1',
            source_kind='snapshot_controlado',
            source_label='https://example.test/stage7?signed_token=secret',
            authorization_ref='Bearer stage7-secret-token',
        )
        issue_codes = {issue['code'] for issue in result['issues']}
        ...
        self.assertFalse(result['ready_for_stage7_reporting'])
        self.assertTrue(result['source_kind_authorized_for_close'])
        self.assertIn('stage7.reporting.source_label_sensitive', issue_codes)
        self.assertIn('stage7.reporting.authorization_ref_sensitive', issue_codes)
        self.assertNotIn('stage7.reporting.source_label_missing', issue_codes)
        self.assertNotIn('stage7.reporting.authorization_ref_missing', issue_codes)
    ...
    def test_annual_reporting_control_sensitive_payload_values_are_blocking(self):
        self._create_valid_local_matrix()
        summary = self._annual_summary()
        ProcesoRentaAnual.objects.update(
    ...
    def test_annual_reporting_control_sensitive_final_refs_are_classified_explicitly(self):
        self._create_valid_local_matrix()
        ProcesoRentaAnual.objects.update(
