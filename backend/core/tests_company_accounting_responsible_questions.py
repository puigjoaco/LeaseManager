...
class CompanyAccountingResponsibleQuestionsTests(SimpleTestCase):
    ...
    def test_command_read_error_does_not_echo_sensitive_path(self):
        with TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            ownership_path = temp_root / 'Socio Controlado Uno 11111111-1.json'
            ownership_path.write_text(json.dumps(self._ownership_validation()), encoding='utf-8')

            with patch.object(
                Path,
                'read_text',
                side_effect=OSError('D:/Privado/Socio Controlado Uno 11111111-1/questions-source.json'),
            ):
                with self.assertRaises(CommandError) as error:
                    call_command(
                        'materialize_company_accounting_responsible_questions',
            ...
            rendered_error = str(error.exception)
            self.assertEqual(rendered_error, 'No se pudo leer ownership_validation JSON.')
            self.assertNotIn('Socio Controlado Uno', rendered_error)
            self.assertNotIn('11111111-1', rendered_error)
    ...
    def test_command_write_error_does_not_echo_sensitive_path(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with patch.object(
                Path,
                'write_text',
                side_effect=OSError('D:/Privado/Socio Controlado Uno 11111111-1/questions.json'),
            ):
                with self.assertRaises(CommandError) as error:
                    call_command(
                        'materialize_company_accounting_responsible_questions',
            ...
            rendered_error = str(error.exception)
            self.assertEqual(rendered_error, 'No se pudo escribir paquete de preguntas responsables.')
            self.assertNotIn('Socio Controlado Uno', rendered_error)
            self.assertNotIn('11111111-1', rendered_error)
            self.assertNotIn('D:/Privado', rendered_error)
