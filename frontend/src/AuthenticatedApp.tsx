...
function App() {
  ...
  async function handleCreateContrato(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = await submitMutation(
      ...
      isEdit ? 'PATCH' : 'POST',
      {
        ...
        ...(contratoDraft.fecha_registro_operativo ? { fecha_registro_operativo: contratoDraft.fecha_registro_operativo } : {}),
  ...
  function startEditContrato(row: Contrato) {
    ...
    setContratoDraft({
      ...
      fecha_registro_operativo: row.fecha_registro_operativo || '',
    ...
    () =>
      mandatos.filter((item) =>
        matches(normalizedSearch, [
          ...
          item.autoridad_operativa_nombre,
          item.autoridad_operativa_rut,
          item.autoridad_operativa_evidencia_ref,
          item.autoriza_recaudacion ? 'autoriza recaudacion' : 'sin recaudacion',
          item.autoriza_facturacion ? 'autoriza facturacion' : 'sin facturacion',
          item.autoriza_comunicacion ? 'autoriza comunicacion' : 'sin comunicacion',
          item.vigencia_desde,
          item.vigencia_hasta,
          item.estado,
        ]),
  ...
  const filteredPagos = useMemo(
    () =>
      pagos.filter((item) =>
        matches(normalizedSearch, [
          item.contrato,
          item.periodo_contractual,
          item.anio,
          item.mes,
          item.estado_pago,
          item.codigo_conciliacion_efectivo,
          item.fecha_vencimiento,
          item.fecha_deposito_banco,
          item.fecha_deteccion_sistema,
