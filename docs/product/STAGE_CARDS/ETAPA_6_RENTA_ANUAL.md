...
experta.
El manifest directo aplica el boundary controlado antes de emitir payload:
`company_ref`, `source_label`, `authorization_ref`, `responsible_ref` y la ruta
relativa de `--output` bajo `local-evidence/` deben ser no sensibles. El CLI no
ecoa `source_root` ni rutas de salida crudas si faltan fuentes o falla la
escritura.
...
candidatos hasheados, conteos de paginas renderizadas, items pendientes y
decision de readiness. La corrida real local AC2025/AT2026 queda
`ready_for_manual_review=true`, con 10 candidatos y 10 candidatos renderizados,
pero `ready_for_controlled_db_load=false` por
`ownership_patch_validation_missing`, `participants_count=0` y falta de patch
...
exportable, paquete materializable `annual-tax-export-file-package-v1` con
...
- `AnnualTaxExport` bloquea readiness si falta, si esta desalineado, si contiene
  ...
  manifiestos o paquete no cubren todos los items DDJJ/F22, si hay revision
  pendiente o si el export, sus contratos, manifiestos o archivos intentan
  declarar formato oficial SII, presentacion SII o calculo fiscal final
  autonomo.
