...
| BLK-009 | La rama `codex/stage1-migration-config-gates` esta empujada pero no integrada a `main` porque `gh` no esta autenticado y el conector GitHub devuelve 403 al crear PR. | requiere_decision_usuario | El paquete local de Etapa 1 queda fuera del root diario hasta PR/CI/merge; seguir agregando codigo local no desbloquea la integracion. | Abrir PR manual, autenticar `gh`/GH_TOKEN o habilitar permisos del conector; luego ejecutar CI, mergear si pasa, hacer pull en `main` y limpiar worktree/rama. | abierto |
