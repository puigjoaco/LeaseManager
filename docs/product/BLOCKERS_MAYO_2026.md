...
| BLK-011 | Cobertura bancaria/leasing externa no puede declararse completa sin manifiesto redactado autorizado y auditado. | bloqueado_externo | Contabilidad/Renta no puede afirmar respaldo bancario/leasing completo por correos, adjuntos o referencias no auditadas. No impide seguir con arquitectura anual, exportadores ni revision segura. | Preparar o autorizar un manifiesto redactado bajo `local-evidence/` o fuente controlada, ejecutar `audit_company_bank_support_coverage` y resolver cualquier operacion con categorias faltantes antes de declarar cobertura completa. | abierto |
