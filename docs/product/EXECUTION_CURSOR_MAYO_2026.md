...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `validate_company_accounting_responsible_answers` bloqueando `company_ref` sensible o no canonico tanto en el paquete de preguntas como en el payload de respuestas, sin filtrar el valor y sin marcar el handoff como listo. Debe mantener la restriccion de outputs versionados, no escribir DB y no declarar revision productiva, ownership final, calculo tributario final ni presentacion SII. |
