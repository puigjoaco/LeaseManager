...
| Fuente exacta | `main` en `48b5f775`, despues del merge confirmado de PR #978 `codex/stage6-answers-draft-audit`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. El tramo preguntas/template/preflight/handoff/materializacion desde handoff y auditoria dry-run de borradores de respuestas responsables ya esta preparado; sigue faltando un `company-accounting-responsible-answers-review.json` listo, que depende de respuestas responsables externas y no debe inventarse desde el sistema. |
...
| Worktree | No hay paquete tactico activo despues de PR #978. Si se abre nuevo trabajo, crear un worktree `codex/...` nuevo desde `origin/main`. No recrear `D:/Proyectos/LeaseManager-stage6-answers-draft-audit` ni la rama `codex/stage6-answers-draft-audit` si ya no existen. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | Ninguna rama tactica activa. Los paquetes cerrados #972-#978 no deben reabrirse por compactacion o `goal_context`. |
...
| Estado al cerrar paquete | No hay paquete abierto. El siguiente paquete valido debe avanzar una brecha nueva y verificable, o documentar explicitamente una pausa segura, sin repetir preguntas/template/preflight/handoff/materializacion desde handoff/auditor dry-run ya cerrados. Debe no leer documentos reales, no escribir DB y no declarar revision productiva, ownership final, calculo tributario final ni presentacion SII. |
