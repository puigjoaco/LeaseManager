...
| Estado | Auditor empresa/ano disponible por comando y expuesto en Reporting como `contabilidad/progreso-empresa/`; candidatos disponibles en `contabilidad/candidatos-progreso-empresa/`. Para Inmobiliaria Puig AC2024/AT2025 se agrego manifiesto read-only y plan de carga controlada de fuentes externas. Balance General, RLI/CPT/RAI, DDJJ y F22 quedan como comparacion, no como calculo. El plan real queda `ready_for_db_load=false` por falta de parser/carga manual controlada para libros anuales, F29 PDF y remuneraciones, mas writer DB local y comparador. |
...
| Siguiente accion | Cerrar `codex/ac2024-mirror-source-bundle` y abrir el siguiente paquete de prueba espejo: parser/carga manual controlada para `annual_ledger_input`, `f29_support_input` y `payroll_support`, luego writer DB local para materializar cierres/hechos AC2024 sin usar outputs esperados como input. |
