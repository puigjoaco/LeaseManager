...
| Fuente exacta | `main` en `a566eed8`, despues del merge confirmado de PR #956 `codex/stage6-mirror-proof-path-redaction`. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar cubierto por tests que `materialize_company_accounting_responsible_answers` y `materialize_company_accounting_responsible_answers_template` no ecoan rutas locales cuando faltan o fallan archivos JSON de entrada, y que la documentacion refleje lectura/escritura como frontera protegida; la salida versionable sigue restringida a `local-evidence/`. |
...
| Siguiente accion | Cerrar `codex/stage6-responsible-answer-read-path-redaction` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, tratarlo como cerrado y continuar con el siguiente tramo de AC2025/AT2026: completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, materializar el workbench con ese review, completar patch ownership/vigencia controlado, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
