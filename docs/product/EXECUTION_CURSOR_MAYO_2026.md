...
| Fuente exacta | `main` en `2e485bda`, despues del merge confirmado de PR #971 `codex/stage6-workbench-summary-redaction`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. No existe `company-accounting-responsible-answers-review.json` listo bajo `local-evidence` en este root, asi que no se debe inventar el handoff ni materializar workbench estricto como si estuviera listo. El tramo actual convierte esa validacion en un auditor reproducible: escanea solo manifests redactados bajo `local-evidence`, no devuelve rutas crudas y reporta si existe exactamente un review responsable listo. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-responsible-review-discovery` en rama `codex/stage6-responsible-review-discovery`, solo mientras no este mergeado. Si `main` ya contiene el merge de este paquete y el worktree/rama no existen, no recrearlo. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-responsible-review-discovery` para auditar presencia/readiness de `company-accounting-responsible-answers-review.json` sin inspeccion manual ni rutas sensibles. |
| Estado | PR #971 ya esta mergeado y con `acceptance` remoto exitoso sobre `2e485bda`. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package, proof anual, plan de carga con errores de ruta redactados, cursor anti-repeticion, contexto/refs superiores no sensibles en reviews, selector de candidatos coherente con proceso anual trazable, `next_action_ref` superior seguro en respuestas responsables y summaries de workbench sanitizados; sigue sin snapshot ownership controlado ni writer anual listo. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `audit_company_accounting_responsible_answers_review_presence` y el comando homonimo auditando manifests `company-accounting-responsible-answers-review.json` bajo `local-evidence`, sin devolver rutas crudas, y distinguiendo `review_missing`, `review_not_ready`, multiples reviews listos o exactamente un handoff listo. Debe no leer documentos reales, no escribir DB y no declarar revision productiva, ownership final, calculo tributario final ni presentacion SII. |
...
| Siguiente accion | Cerrar `codex/stage6-responsible-review-discovery` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, continuar con el siguiente tramo de AC2025/AT2026 desde evidencia autorizada: ejecutar el auditor de review responsable; si no existe review listo o conserva blockers, no inventarlo y avanzar solo con preparacion segura. Cuando exista exactamente un review responsable listo, ejecutar `materialize_company_accounting_responsible_answers --fail-on-blocking`, materializar el workbench con `--require-responsible-answers-ready`, completar patch ownership/vigencia controlado al 31-12-2025, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
