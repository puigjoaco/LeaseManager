...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `validate_company_accounting_responsible_answers` bloqueando `evidence_ref` y `next_action_ref` sensibles a nivel superior del payload, sin filtrar el valor y sin marcar el handoff como listo. Debe mantener la restriccion de outputs versionados, no escribir DB y no declarar revision productiva, ownership final, calculo tributario final ni presentacion SII. |
