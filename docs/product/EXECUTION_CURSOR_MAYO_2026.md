...
| Fuente exacta | `main` en `d8a11d28`, despues del merge confirmado de PR #968 `codex/stage6-responsible-answer-top-ref-guards`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. No existe `company-accounting-responsible-answers-review.json` listo bajo `local-evidence` en este root, asi que no se debe inventar el handoff ni materializar workbench estricto como si estuviera listo. El tramo actual endurece el selector de candidatos de progreso contable/renta: los artefactos anuales downstream solo pueden sumar senales si pertenecen a un `ProcesoRentaAnual` preparado o superior y trazado por `AnnualTaxSourceBundle` congelado. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-company-progress-candidate-annual-process` en rama `codex/company-progress-candidate-annual-process`, solo mientras no este mergeado. Si `main` ya contiene el merge de este paquete y el worktree/rama no existen, no recrearlo. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/company-progress-candidate-annual-process` para impedir que el selector de candidatos infle avance anual con artefactos downstream que no pertenecen a un proceso anual valido y trazable. |
| Estado | PR #968 ya esta mergeado y con `acceptance` remoto exitoso sobre `d8a11d28`. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package, proof anual, plan de carga con errores de ruta redactados, cursor anti-repeticion y contexto/refs superiores no sensibles en reviews; sigue sin snapshot ownership controlado ni writer anual listo. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `collect_company_accounting_candidates` contando `annual_trial_balance`, `rli_cpt_workbooks`, `annual_dossier` y `annual_export` solo desde artefactos asociados a procesos anuales preparados o superiores con `AnnualTaxSourceBundle` congelado. Debe mantener senales mensuales y boundary de seleccion sin usar fuentes externas ni declarar revision productiva, ownership final, calculo tributario final o presentacion SII. |
...
| Siguiente accion | Cerrar `codex/company-progress-candidate-annual-process` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, continuar con el siguiente tramo de AC2025/AT2026 desde evidencia autorizada: validar si existe un `company-accounting-responsible-answers-review.json` completo y no sensible; si no existe o conserva blockers, no inventarlo y avanzar solo con preparacion segura. Cuando exista review responsable listo, ejecutar `materialize_company_accounting_responsible_answers --fail-on-blocking`, materializar el workbench con `--require-responsible-answers-ready`, completar patch ownership/vigencia controlado al 31-12-2025, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
