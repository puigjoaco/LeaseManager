...
| Fuente exacta | `main` posterior al cierre del paquete `stage6-artifact-warning-review`. |
| Brecha activa | No hay paquete tactico abierto para revision de warnings de matriz DDJJ/F22. `AnnualTaxArtifactMatrixItem.warning_review_ref` ya permite registrar revision responsable no sensible sin borrar warnings ni declarar calculo final. |
...
| Estado | El flujo anual distingue warnings totales de warnings pendientes de revision: un item DDJJ/F22 con warnings solo deja de bloquear dossier/export/checklist si esta `listo_revision`, conserva `warning_review_ref` no sensible y los artefactos derivados fueron regenerados. Esto mantiene `final_tax_calculation=false`, `official_format=false` y `sii_submission=false`. |
