...
| Fuente exacta | `main` en `94788c8c`, despues del merge confirmado de PR #983 `codex/cursor-post-982`. |
| Brecha activa | Paquete tactico `codex/bank-support-formal-review`: separar soporte bancario/leasing observado (`expected_complete`) de soporte formal (`verified_complete`) para que el intake local siga siendo revisable, pero el paquete contable/renta no quede productivamente listo sin confirmacion formal. Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. |
...
| Worktree | Paquete en cierre: `D:/Proyectos/LeaseManager-bank-support-formal-review` en rama `codex/bank-support-formal-review`. Si este paquete ya fue mergeado y el worktree no existe, tratarlo como cerrado y no recrearlo. No recrear `D:/Proyectos/LeaseManager-company-progress-source-bundle`, `D:/Proyectos/LeaseManager-company-progress-review-boundary` ni sus ramas `codex/...` cerradas. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/bank-support-formal-review` hasta merge. Los paquetes cerrados #972-#983 y este paquete, si `main` ya contiene su merge, no deben reabrirse por compactacion, resumen, chat antiguo ni `goal_context`. |
...
| Siguiente accion | Si `codex/bank-support-formal-review` sigue activo, terminarlo con PR/CI/merge/limpieza. Si `main` ya contiene su merge, no recrearlo: volver al flujo normal del frente AC2025/AT2026, auditar primero un archivo externo de respuestas responsables si existe; si no existe, no pedirlo en bucle ni inventarlo, y elegir solo trabajo local seguro independiente de la respuesta responsable. No crear PRs para reexplicar el goal ni para repetir paquetes #972-#983. |
