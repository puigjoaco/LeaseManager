...
| Campo | Valor |
| --- | --- |
| Frente activo | Paquete tactico abierto: alinear codigos de configuracion fiscal anual Reporting con readiness Etapa 7. |
| Fuente exacta | `main` limpio `e8e8247c` tras PR #794; brecha detectada al comparar `_assert_annual_tax_traceability()` con `audit_stage7_reporting_readiness`. Rescue queda pausado fuera de alcance. |
| Brecha activa | `_assert_annual_tax_traceability()` agrupa falta de `ConfiguracionFiscalEmpresa` bajo un codigo generico; readiness ya clasifica proceso anual, DDJJ y F22 por separado. |
...
| Worktree | `D:/Proyectos/LeaseManager-stage7-annual-fiscal-config-codes`. |
| Rama | `codex/stage7-annual-fiscal-config-codes`. |
