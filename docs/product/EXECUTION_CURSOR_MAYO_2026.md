...
| Fuente exacta | `main` en `d8dfd6d8`, despues del merge confirmado de PR #973 `codex/stage6-responsible-handoff-preflight`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. Ya existe preflight reproducible que confirma preguntas y template listos, pero sin `company-accounting-responsible-answers-review.json`; el tramo actual materializa un handoff packet verificable para agrupar preguntas + template sin depender de memoria del chat ni rutas crudas. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-responsible-handoff-packet` en rama `codex/stage6-responsible-handoff-packet`, solo mientras no este mergeado. Si `main` ya contiene el merge de este paquete y el worktree/rama no existen, no recrearlo. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-responsible-handoff-packet` para materializar un paquete local verificable de preguntas + template responsable AC2025/AT2026, sin inspeccion manual ni rutas sensibles. |
| Estado | PR #973 ya esta mergeado y con `acceptance` remoto exitoso sobre `d8dfd6d8`. El preflight desde `main` confirma 1 paquete de preguntas, 1 template rellenable, 0 reviews responsables, `ready_for_responsible_answer_completion=true` y `ready_for_responsible_decision_handoff=false`. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package, proof anual, plan de carga con errores de ruta redactados, cursor anti-repeticion, contexto/refs superiores no sensibles en reviews, selector de candidatos coherente con proceso anual trazable, `next_action_ref` superior seguro en respuestas responsables, summaries de workbench sanitizados, auditor de discovery de review y preflight de handoff; sigue sin review responsable listo, snapshot ownership controlado ni writer anual listo. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `materialize_company_accounting_responsible_handoff_packet`, `build/write/verify_company_accounting_responsible_handoff_packet` y tests para agrupar `company-accounting-responsible-questions.json` + `company-accounting-responsible-answers.template.json` en una carpeta verificable con `company-accounting-responsible-handoff-packet.json`, sin rutas crudas. Debe quedar como entrega lista para completar respuestas responsables, no como review listo. Debe no leer documentos reales, no escribir DB y no declarar revision productiva, ownership final, calculo tributario final ni presentacion SII. |
