...
| Fuente exacta | `main` en `6fee9099`, despues del merge confirmado de PR #984 `codex/bank-support-formal-review`. |
| Brecha activa | Paquete tactico `codex/document-intake-formal-bank-support`: alinear `company_document_intake` con el boundary de PR #984 para que `expected_complete` habilite revision documental local, pero no `ready_for_productive_document_review` sin confirmacion bancaria/leasing `verified_complete`. Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. |
...
| Worktree | Activo: `D:/Proyectos/LeaseManager-document-intake-formal-bank-support` en rama `codex/document-intake-formal-bank-support`. Si este paquete ya fue mergeado y el worktree no existe, tratarlo como cerrado y no recrearlo. No recrear `D:/Proyectos/LeaseManager-bank-support-formal-review`, `D:/Proyectos/LeaseManager-company-progress-source-bundle`, `D:/Proyectos/LeaseManager-company-progress-review-boundary` ni sus ramas `codex/...` cerradas. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/document-intake-formal-bank-support` hasta merge. Los paquetes cerrados #972-#984 y este paquete, si `main` ya contiene su merge, no deben reabrirse por compactacion, resumen, chat antiguo ni `goal_context`. |
...
| Siguiente accion | Cerrar `codex/document-intake-formal-bank-support` con PR/CI/merge/limpieza. Si `main` ya contiene el merge y este worktree no existe, tratarlo como cerrado y continuar con el siguiente frente seguro desde este cursor. No crear PRs para reexplicar el goal ni para repetir paquetes #972-#984. |
