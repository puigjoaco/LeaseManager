...
| Fuente exacta | `main` en `2c058849`, despues del merge confirmado de PR #957 `codex/stage6-responsible-answer-read-path-redaction`. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-workbench-responsible-answers-gate` en rama `codex/stage6-workbench-responsible-answers-gate`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-workbench-responsible-answers-gate` para agregar modo estricto al workbench ownership cuando se requiere review de respuestas responsables listo. |
| Estado | PR #957 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package y proof anual con errores de ruta redactados, pero sigue sin snapshot ownership controlado ni writer anual listo. Este paquete evita que una corrida estricta del workbench cree borrador privado si falta el review de respuestas responsables o conserva blockers. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `materialize_annual_tax_ownership_patch_workbench --require-responsible-answers-ready` fallando antes de escribir cuando falta `company-accounting-responsible-answers-review.json` listo o conserva blockers, manteniendo el modo no estricto para preparar borradores privados observados; la salida versionable sigue restringida a `local-evidence/`. |
...
| Siguiente accion | Cerrar `codex/stage6-workbench-responsible-answers-gate` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, tratarlo como cerrado y continuar con el siguiente tramo de AC2025/AT2026: completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, materializar el workbench con ese review usando `--require-responsible-answers-ready`, completar patch ownership/vigencia controlado, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
