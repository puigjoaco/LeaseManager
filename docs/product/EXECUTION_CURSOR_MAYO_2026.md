...
| Fuente exacta | `main` en `e090874b`, despues del merge confirmado de PR #997 `codex/stage6-mirror-proof-run-evidence`. |
| Brecha activa | Paquete tactico `codex/stage6-proof-artifact-link`: cerrar el bypass residual por el cual `audit_annual_tax_mirror_proof` podia recibir un `mirror_run` valido, pero no cruzar sus `process_id` y `source_bundle_id` con los artefactos generados que el comparador evalua. El proof debe bloquear si la corrida mirror no corresponde exactamente al `ProcesoRentaAnual` y `AnnualTaxSourceBundle` del inventario generado. Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. |
...
| Estado al cerrar paquete | En curso: este paquete hace que `audit_annual_tax_mirror_proof` enlace la evidencia del mirror con los artefactos comparados. La salida agrega `mirror_run_artifact_link_summary` sanitizado y bloquea si `process_id` o `source_bundle_id` del run no coinciden con `generated_inventory` y `comparison_generated_artifact_evidence`. Mantiene `responsible_review_gate` como boundary de revision responsable y no lee `.env`, DB real, SII real, banco, EDIG ejecutable, correos ni integraciones externas. |
...
| Siguiente accion | Cerrar `codex/stage6-proof-artifact-link` con validaciones proporcionales, PR/CI/merge/limpieza. Si `main` ya contiene el merge y este worktree no existe, tratarlo como cerrado y continuar con el siguiente frente seguro desde este cursor. No crear PRs para reexplicar el goal ni para repetir paquetes #972-#997. |
