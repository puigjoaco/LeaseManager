...
| Fuente exacta | `main` en `1e8b012a`, despues del merge confirmado de PR #976 `codex/stage6-answers-from-handoff`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. El tramo preguntas/template/preflight/handoff/materializacion desde handoff ya esta preparado; sigue faltando un `company-accounting-responsible-answers-review.json` listo, que depende de respuestas responsables externas y no debe inventarse desde el sistema. |
...
| Estado al cerrar paquete | No hay paquete abierto. El siguiente paquete valido debe avanzar una brecha nueva y verificable, o documentar explicitamente una pausa segura, sin repetir preguntas/template/preflight/handoff/materializacion desde handoff ya cerrados. Debe no leer documentos reales, no escribir DB y no declarar revision productiva, ownership final, calculo tributario final ni presentacion SII. |
...
| Siguiente accion | Si existe un archivo externo de respuestas responsables redactadas, ejecutar `materialize_company_accounting_responsible_answers --handoff-packet-dir <packet> --answers <answers.json> --fail-on-blocking` y luego, si queda exactamente un review responsable listo, materializar el workbench con `--require-responsible-answers-ready`, completar patch ownership/vigencia controlado al 31-12-2025, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. Si no existe ese archivo, no pedirlo en bucle ni inventarlo: elegir solo trabajo local seguro e independiente de la respuesta responsable, o pausar el frente con este estado. |
