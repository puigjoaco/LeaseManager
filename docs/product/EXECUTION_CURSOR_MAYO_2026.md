...
| Fuente exacta | `main` en `98a71517`, despues del merge confirmado de PR #980 `codex/company-progress-review-boundary`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. El tramo preguntas/template/preflight/handoff/materializacion desde handoff, auditoria dry-run de borradores de respuestas responsables y exposicion de `responsible_review_gate` en progreso/reporting ya esta preparado. Sigue sin existir review responsable listo, snapshot ownership controlado ni writer anual listo. |
...
| Estado al cerrar paquete | El paquete #980 quedo cerrado: `audit_company_accounting_progress --empresa-id <id> --fiscal-year <ano>` reporta `responsible_review_gate`; con fases locales incompletas apunta a la siguiente fase local, y con fases locales completas exige review responsable externo/controlado mientras mantiene en falso handoff responsable, calculo tributario final y presentacion SII. No se leyo `.env`, DB real, SII real, banco, EDIG ejecutable, correos ni integraciones externas. |
