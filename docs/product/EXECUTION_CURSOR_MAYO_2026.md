...
| Estado | Auditor empresa/ano disponible por comando y expuesto en Reporting como `contabilidad/progreso-empresa/`; candidatos disponibles en `contabilidad/candidatos-progreso-empresa/`. Para Inmobiliaria Puig AC2024/AT2025 se agrego manifiesto read-only de fuentes externas que separa input contable/soportes de outputs esperados SII. Pendiente carga controlada a DB local, generacion de artefactos LeaseManager y comparacion contra F22/DDJJ/balance definitivos AT2025. |
