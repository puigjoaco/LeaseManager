...
| Worktree | Ninguno activo esperado despues de integrar el paquete de regimen soportado; `main` queda como base limpia. |
| Rama | `main`; abrir `codex/...` solo cuando el siguiente paquete concreto lo requiera. |
| Estado | Auditor empresa/ano disponible por comando y expuesto en Reporting como `contabilidad/progreso-empresa/`; candidatos disponibles en `contabilidad/candidatos-progreso-empresa/`. Ambos conservan senales locales pero bloquean review/renta si la configuracion fiscal activa esta fuera de `EmpresaContabilidadCompletaV1`. Pendiente seleccion de empresa piloto y fuente de ejecucion. |
