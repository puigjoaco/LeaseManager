...
| Fuente exacta | `main` en `f326e202`, despues del merge confirmado de PR #995 `codex/controlled-db-load-review-sources`. |
| Brecha activa | Paquete tactico `codex/stage6-mirror-ownership-gate`: cerrar el bypass por el cual `run_annual_tax_controlled_mirror` podia declarar `ready_for_generation=true` y generar capa anual con 12 `MonthlyTaxFact` aunque faltara snapshot ownership vigente al 31-12 del ano comercial. El mirror debe reportar `ownership_snapshot` sanitizado, bloquear con `ownership_snapshot_missing`, `ownership_snapshot_incomplete` o `ownership_snapshot_duplicate_participants`, y no escribir ProcesoRentaAnual/DDJJ/F22 si falta esa condicion. Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. |
...
| Estado al cerrar paquete | En curso: este paquete hace que `run_annual_tax_controlled_mirror` replique el gate de readiness anual del paquete controlado: 12 meses no bastan si falta ownership vigente 31-12. La salida agrega `ownership_snapshot` sanitizado y bloquea generacion anual cuando falta, esta incompleto o tiene duplicados. Mantiene `responsible_review_gate` como boundary de revision responsable y no lee `.env`, DB real, SII real, banco, EDIG ejecutable, correos ni integraciones externas. |
...
| Siguiente accion | Cerrar `codex/stage6-mirror-ownership-gate` con validaciones proporcionales, PR/CI/merge/limpieza. Si `main` ya contiene el merge y este worktree no existe, tratarlo como cerrado y continuar con el siguiente frente seguro desde este cursor. No crear PRs para reexplicar el goal ni para repetir paquetes #972-#995. |
