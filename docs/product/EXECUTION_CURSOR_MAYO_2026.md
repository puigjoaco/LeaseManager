...
| Estado | Auditor empresa/ano disponible por comando y expuesto en Reporting como `contabilidad/progreso-empresa/`; candidatos disponibles en `contabilidad/candidatos-progreso-empresa/`. Para Inmobiliaria Puig AC2024/AT2025 se agrego manifiesto read-only, plan de carga controlada y writer DB local `apply_annual_tax_controlled_db_load` para paquetes normalizados. Balance General, RLI/CPT/RAI, DDJJ y F22 quedan como comparacion, no como calculo. El plan real queda `ready_for_db_load=false` por falta de parser/carga manual controlada para libros anuales, F29 PDF y remuneraciones, mas comparador contra outputs esperados. |
...
| Siguiente accion | Cerrar `codex/ac2024-mirror-source-bundle` y abrir el siguiente paquete de prueba espejo: parser/carga manual controlada para `annual_ledger_input`, `f29_support_input` y `payroll_support`, aplicar writer DB local sobre paquete normalizado y luego implementar comparador contra outputs esperados sin usarlos como input. |
