...
| Estado | Auditor empresa/ano disponible por comando y expuesto en Reporting como `contabilidad/progreso-empresa/`; candidatos disponibles en `contabilidad/candidatos-progreso-empresa/`. Para Inmobiliaria Puig AC2024/AT2025 se agrego manifiesto read-only, plan de carga controlada, template de paquete normalizado y writer DB local `apply_annual_tax_controlled_db_load`. Balance General, RLI/CPT/RAI, DDJJ y F22 quedan como comparacion, no como calculo. El template real generado bajo `local-evidence/` confirma refs mensuales completas (`monthly_input_refs_complete=true`) y libros anuales presentes; F29 febrero/diciembre quedan como `no_aplica` por meses sin declaracion. El plan queda `ready_for_db_load=false` hasta completar valores normalizados desde libros anuales, F29/meses sin declaracion y remuneraciones, mas comparador contra outputs esperados. |
...
| Siguiente accion | Cerrar `codex/ac2024-mirror-source-bundle` y abrir el siguiente paquete de prueba espejo: completar el template de carga con parser/carga manual controlada para `annual_ledger_input`, `f29_support_input` y `payroll_support`, aplicar writer DB local sobre paquete normalizado y luego implementar comparador contra outputs esperados sin usarlos como input. |
