...
| Fuente exacta | `main` en `922945f0`, despues del merge confirmado de PR #954 `codex/stage6-package-readiness-path-redaction`. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-mirror-comparison-path-redaction` en rama `codex/stage6-mirror-comparison-path-redaction`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-mirror-comparison-path-redaction` para redactar errores de archivo de `run_annual_tax_controlled_mirror` y `compare_annual_tax_expected_outputs`. |
| Estado | PR #954 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, workbench, preguntas, respuestas y review package con errores de ruta redactados, pero sigue sin snapshot ownership controlado ni writer anual listo. Este paquete protege el borde mirror/comparacion para que errores de lectura/escritura no impriman rutas privadas con nombres o RUTs. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `run_annual_tax_controlled_mirror` y `compare_annual_tax_expected_outputs` sin eco de rutas locales cuando falla la escritura del run/comparacion o falta/falla la lectura del manifest de comparacion; la salida versionable sigue restringida a `local-evidence/`. |
...
| Siguiente accion | Cerrar `codex/stage6-mirror-comparison-path-redaction` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, tratarlo como cerrado y continuar con el siguiente tramo: completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, materializar el workbench con ese review, completar patch ownership/vigencia controlado, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
