...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-workbench-review-consistency` en rama `codex/stage6-workbench-review-consistency`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-workbench-review-consistency` para derivar readiness efectiva del review de respuestas dentro del workbench ownership y rechazar inconsistencias antes de escribir en modo estricto. |
| Estado | PR #962 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package y proof anual con errores de ruta redactados, pero sigue sin snapshot ownership controlado ni writer anual listo. El paquete #962 impidio que un review parcial generado por `--allow-incomplete` se marque listo. El paquete actual cierra la via de un review externo inconsistente que declara listo pero conserva faltantes o blockers. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `build_annual_tax_ownership_patch_workbench` y `materialize_annual_tax_ownership_patch_workbench --require-responsible-answers-ready` derivando `responsible_answers_ready=false` cuando el review de respuestas tenga `missing_question_keys`, `missing_questions_total>0`, `blocking_issues_total>0`, issue codes o menos respuestas que preguntas, aunque el review declare `ready_for_responsible_decision_handoff=true`. Debe mantener la redaccion de errores, la restriccion `local-evidence/` y no declarar ownership final, calculo tributario final ni presentacion SII. |
...
| Siguiente accion | Cerrar `codex/stage6-workbench-review-consistency` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, continuar con el siguiente tramo de AC2025/AT2026: completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, materializar el workbench con ese review usando `--require-responsible-answers-ready`, completar patch ownership/vigencia controlado al 31-12-2025, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
