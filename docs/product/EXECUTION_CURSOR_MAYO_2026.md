...
| Brecha activa | El draft AC2024 elegia el ultimo `annual_ledger_input` por `artifact_key` y podia tomar fuentes de `Ano_2025` o historicas. Corregir selector, regenerar paquete con ownership controlado y dejar el gate Etapa 6 reducido a brechas reales. |
| Motivo de prioridad | Evita que una fuente anual contaminada por carpetas posteriores desvie la prueba espejo y permite saber con precision que falta para cerrar Inmobiliaria Puig AC2024/AT2025. |
...
| Estado actualizado | PR #856 ya esta mergeado en `main`. En este worktree el selector de `annual_ledger_input` fue corregido para priorizar `commercial_year` y rutas canonicas. La regeneracion real AC2024/AT2025 queda con 180 campos, 0 errores de extraccion y `ready_for_db_writer=true`. Con paquete local ignorado que incluye `ownership` controlado desde evidencia societaria revisada, `audit_annual_tax_controlled_package_readiness` queda `ready_for_annual_generation=true`; `apply_annual_tax_controlled_db_load --apply` carga 12 meses y 1 participante; `run_annual_tax_controlled_mirror --apply` genera ProcesoRentaAnual, DDJJ/F22 preparados, matriz, dossier, export y checklist con source bundle `snapshot_controlado`. Gate Etapa 6 queda `classification=parcial`, `ready_for_stage6_renta_anual=false` solo por `stage6.real_estate_item_missing` y `stage6.tax_support_document_missing`. |
...
| Siguiente accion | Cerrar PR/CI/merge de `codex/ac2024-ownership-patch`. El siguiente paquete de producto debe atacar uno de los dos blockers restantes de Etapa 6 para Inmobiliaria Puig AC2024/AT2025: materializar `AnnualRealEstateItem` desde propiedad/arriendos/contribuciones controladas o crear/cargar `TaxSupportDocument` usable. No reabrir ownership, DDJJ/F22 semantico, Balance General, RLI/CPT/RAI/SAC, EDIG ni goal prompts como bloqueo general salvo bug nuevo. |
