...
| Brecha activa | Ninguna. Ultimo paquete integrado: Etapa 2 / Canales normaliza refs/textos operativos antes de `full_clean()` y `save()` en gates, configuraciones, mensajes y notificaciones. |
| Motivo de prioridad | Paquete local validado e integrado por PR #757; Canales conserva refs canonicas antes de validadores de campo, persistencia, snapshot, readiness y auditoria. |
...
| Estado al cerrar paquete | PR #757 mergeado en `main` (`04771c09`) tras CI remoto `acceptance` exitoso. Validacion local ejecutada: focal Canales 1 test, suite impactada Canales/readiness 180 tests, `manage.py check`, migraciones dry-run, gate Etapa 2 local `classification=parcial`, `npm ci`, `npm run build`, `npm run lint`, acceptance local 1298 tests, higiene repo y `git diff --check`. |
