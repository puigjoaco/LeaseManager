...
| Fuente exacta | `main` en `95a938f0`, despues del merge confirmado de PR #987 `codex/audit-review-package-document-intake`. |
| Brecha activa | Paquete tactico `codex/reporting-review-package-source-boundary`: proteger el endpoint Reporting de paquete de revision contable/renta para que rechace rutas o paquetes locales de intake documental enviados por HTTP, manteniendo esa verificacion solo en comandos CLI con paquete en disco y evitando que el API ignore campos peligrosos. Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. |
...
| Worktree | Activo: `D:/Proyectos/LeaseManager-reporting-review-package-source-boundary` en rama `codex/reporting-review-package-source-boundary`. Si este paquete ya fue mergeado y el worktree no existe, tratarlo como cerrado y no recrearlo. No recrear `D:/Proyectos/LeaseManager-audit-review-package-document-intake`, `D:/Proyectos/LeaseManager-review-package-document-intake-readiness`, `D:/Proyectos/LeaseManager-document-intake-formal-bank-support`, `D:/Proyectos/LeaseManager-bank-support-formal-review`, `D:/Proyectos/LeaseManager-company-progress-source-bundle`, `D:/Proyectos/LeaseManager-company-progress-review-boundary` ni sus ramas `codex/...` cerradas. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/reporting-review-package-source-boundary` hasta merge. Los paquetes cerrados #972-#987 y este paquete, si `main` ya contiene su merge, no deben reabrirse por compactacion, resumen, chat antiguo ni `goal_context`. |
...
| Estado al cerrar paquete | PR #987 quedo cerrado: el auditor dry-run acepta `--document-intake-package-dir`, verifica el intake en disco, preserva hash/readiness y bloquea con `document_intake_not_productive_ready` cuando el intake no esta productivamente listo. Este paquete cierra la frontera HTTP equivalente: Reporting recibe solo `bank_support_manifest` JSON redactado y rechaza `document_intake_package_dir`, `document_intake_package_path` o `document_intake_package` para no leer rutas locales ni confiar en paquetes declarados por cliente. Validado localmente con focal, impactada, checks, gate Etapa 6, frontend y acceptance; pendiente PR/CI/merge/limpieza. Mantiene `responsible_review_gate` como boundary de revision responsable y no lee `.env`, DB real, SII real, banco, EDIG ejecutable, correos ni integraciones externas. |
...
| Siguiente accion | Cerrar `codex/reporting-review-package-source-boundary` con PR/CI/merge/limpieza. Si `main` ya contiene el merge y este worktree no existe, tratarlo como cerrado y continuar con el siguiente frente seguro desde este cursor. No crear PRs para reexplicar el goal ni para repetir paquetes #972-#987. |
