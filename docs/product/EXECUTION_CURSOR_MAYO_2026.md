...
| Fuente exacta | `main` en `691e78c8`, despues del merge confirmado de PR #960 `codex/stage6-package-ownership-snapshot-date`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. El tramo actual endurece el workbench privado ownership para que `responsible_ref` y `approval_ref` no puedan contener RUTs, rutas locales, URLs, correos, secretos ni texto libre antes de escribir el patch privado. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-ownership-workbench-safe-refs` en rama `codex/stage6-ownership-workbench-safe-refs`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-ownership-workbench-safe-refs` para exigir refs no sensibles en el workbench privado ownership antes de generar `ownership-patch-draft.private.json`. |
| Estado | PR #960 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package y proof anual con errores de ruta redactados, pero sigue sin snapshot ownership controlado ni writer anual listo. El paquete #960 impidio que un paquete controlado armado manualmente o heredado habilite generacion anual/escritura local con ownership que no sea foto de cierre anual o cuyos participantes no esten vigentes al 31-12. El paquete actual cierra una contaminacion previa al patch: los refs de responsable/aprobacion del workbench deben ser refs no sensibles o el comando falla antes de escribir. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `build_annual_tax_ownership_patch_workbench` y `materialize_annual_tax_ownership_patch_workbench` rechazando `responsible_ref` o `approval_ref` sensibles antes de escribir manifest o patch privado. Debe mantener el modo `--require-responsible-answers-ready`, la redaccion de errores, la restriccion `local-evidence/` y no declarar ownership final, calculo tributario final ni presentacion SII. |
...
| Siguiente accion | Cerrar `codex/stage6-ownership-workbench-safe-refs` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, continuar con el siguiente tramo de AC2025/AT2026: completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, materializar el workbench con ese review usando `--require-responsible-answers-ready`, completar patch ownership/vigencia controlado al 31-12-2025, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
