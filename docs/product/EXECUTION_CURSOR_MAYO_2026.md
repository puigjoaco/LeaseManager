...
| Fuente exacta | `main` en `a6eeef0b`, despues del merge confirmado de PR #941 `codex/stage6-ownership-patch-injector`. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-ownership-patch-workbench` en rama `codex/stage6-ownership-patch-workbench`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-ownership-patch-workbench` para materializar el espacio privado donde completar ownership patch sin versionar PII. |
| Estado | PR #941 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales y checklist ownership con indice visual disponibles, pero sigue sin snapshot ownership controlado ni writer anual listo. Este paquete agrega un workbench que crea manifest redactado y patch privado rellenable bajo `local-evidence/`, sin leer documentos reales ni imprimir PII. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `materialize_annual_tax_ownership_patch_workbench` disponible por codigo y comando, probado con stdout redactado, guards `local-evidence/`, salida a directorio limpio, patch privado no versionable y documentacion. Despues corresponde completar el patch ownership/vigencia real en ese workbench privado, validarlo, inyectarlo al paquete y continuar con paquete normalizado AC2025/AT2026. |
