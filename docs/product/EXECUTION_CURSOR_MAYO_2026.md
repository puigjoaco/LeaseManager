...
| Fuente exacta | `main` en `cc9eec50`, despues del merge confirmado de PR #970 `codex/stage6-responsible-answer-next-action-fallback`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. No existe `company-accounting-responsible-answers-review.json` listo bajo `local-evidence` en este root, asi que no se debe inventar el handoff ni materializar workbench estricto como si estuviera listo. El tramo actual endurece `materialize_annual_tax_ownership_patch_workbench`: aunque reciba checklist o review responsable contaminados/forjados, el manifest solo puede reflejar keys de resumen sanitizadas y no rutas, nombres, RUTs ni refs sensibles. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-workbench-summary-redaction` en rama `codex/stage6-workbench-summary-redaction`, solo mientras no este mergeado. Si `main` ya contiene el merge de este paquete y el worktree/rama no existen, no recrearlo. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-workbench-summary-redaction` para sanitizar keys de resumen en el workbench ownership cuando consume checklist o review responsable. |
| Estado | PR #970 ya esta mergeado y con `acceptance` remoto exitoso sobre `cc9eec50`. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package, proof anual, plan de carga con errores de ruta redactados, cursor anti-repeticion, contexto/refs superiores no sensibles en reviews, selector de candidatos coherente con proceso anual trazable y `next_action_ref` superior seguro en respuestas responsables; sigue sin snapshot ownership controlado ni writer anual listo. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `build_annual_tax_ownership_patch_workbench` sanitizando `blocking_item_keys`, `validation_blockers`, `decision_states`, `categories` e `issue_codes` antes de escribir el manifest. Debe conservar readiness efectiva, no copiar respuestas crudas, no escribir DB y no declarar revision productiva, ownership final, calculo tributario final ni presentacion SII. |
