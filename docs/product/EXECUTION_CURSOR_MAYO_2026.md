...
| Campo | Valor |
| --- | --- |
| Frente activo | PlataformaBase/Auth - atomicidad de tokens y auditoria en login/logout. |
| Fuente exacta | Estado real de `main` en `c7c9039` despues de PR #619, stage cards, trazabilidad, evidencia y bloqueos vigentes. |
| Brecha activa | `backend/users/views.py` crea tokens de login o elimina tokens de logout y luego registra auditoria fuera de la misma transaccion. Si falla esa auditoria, puede quedar un token creado o eliminado sin traza del endpoint que ejecuto la mutacion. |
...
| Worktree | `D:/Proyectos/LeaseManager-auth-token-audit-atomicity`. |
| Rama | `codex/auth-token-audit-atomicity`. |
| Estado | Paquete tactico abierto. |
| Gate esperado | Tests focales de rollback ante falla de auditoria, suite impactada Users/Auth, `manage.py check`, migraciones dry-run, frontend build/lint, acceptance local, higiene y CI GitHub. |
