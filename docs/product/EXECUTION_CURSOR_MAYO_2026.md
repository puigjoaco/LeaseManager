...
| Fuente exacta | `main` en `154ea044`, despues del merge confirmado de PR #981 `codex/cursor-post-980`. |
| Brecha activa | Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. El tramo preguntas/template/preflight/handoff/materializacion desde handoff, auditoria dry-run de borradores de respuestas responsables y exposicion de `responsible_review_gate` en progreso/reporting ya esta preparado. El paquete actual endurece el medidor de progreso para que artefactos anuales downstream solo cuenten si usan el mismo `AnnualTaxSourceBundle` congelado del `ProcesoRentaAnual`, evitando progreso inflado por artefactos heredados inconsistentes. Sigue sin existir review responsable listo, snapshot ownership controlado ni writer anual listo. |
