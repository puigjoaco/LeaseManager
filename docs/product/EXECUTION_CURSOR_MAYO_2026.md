...
| Fuente exacta | `main` en `c8328531`, despues del merge confirmado de PR #945 `codex/stage6-responsible-answers`. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-responsible-answer-template` en rama `codex/stage6-responsible-answer-template`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-responsible-answer-template` para materializar un template seguro de respuestas responsables desde el paquete de preguntas contables/renta. |
| Estado | PR #945 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales y checklist ownership con indice visual disponibles, pero sigue sin snapshot ownership controlado ni writer anual listo. Este paquete genera `company-accounting-responsible-answers.template.json` desde preguntas redactadas, mantiene decisiones en `pendiente` como bloqueo y evita que el handoff vuelva a texto libre o chat. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar un builder y comando de template de respuestas responsables desde `company-accounting-responsible-questions.json`, con salida solo bajo `local-evidence/` si queda dentro del repo, sin guardar texto libre, nombres, RUTs, rutas locales ni refs sensibles, y con pruebas que demuestren que `pendiente` no cuenta como handoff listo. Despues corresponde completar ese template mediante revision responsable privada, validarlo y usarlo junto al workbench AC2025/AT2026 para completar ownership/vigencia real, Banco Chile/leasing, criterios tributarios y documentos faltantes. |
...
| Siguiente accion | Cerrar `codex/stage6-responsible-answer-template` con PR/CI/merge/limpieza. Luego completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, usarlo junto al workbench AC2025/AT2026, completar patch ownership/vigencia controlado, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
