...
| Fuente exacta | `main` en `780df90d`, despues del merge confirmado de PR #958 `codex/stage6-workbench-responsible-answers-gate`. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-ownership-snapshot-date` en rama `codex/stage6-ownership-snapshot-date`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-ownership-snapshot-date` para exigir que el patch ownership/vigencia controlado sea snapshot de cierre anual al 31-12 del ano comercial. |
| Estado | PR #958 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, preguntas/respuestas responsables, workbench privado conectado, consistencia ownership handoff/writer protegida, writer controlado, readiness, mirror/comparacion, workbench, preguntas, respuestas, review package y proof anual con errores de ruta redactados, pero sigue sin snapshot ownership controlado ni writer anual listo. Este paquete impide que `validate_annual_tax_ownership_patch` e inyeccion acepten un ownership que no sea foto de cierre anual o cuyos participantes no esten vigentes al 31-12. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `validate_annual_tax_ownership_patch` fallando con `ownership_patch_invalid` cuando `ownership.as_of` no sea el 31-12 del ano comercial o algun participante no cubra esa fecha. `inject_annual_tax_ownership_patch_into_controlled_package` debe rechazar esos patches antes de escribir el paquete. |
...
| Siguiente accion | Cerrar `codex/stage6-ownership-snapshot-date` con PR/CI/merge/limpieza. Si `main` ya contiene ese merge y el worktree/rama no existen, continuar con el siguiente tramo de AC2025/AT2026: completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, materializar el workbench con ese review usando `--require-responsible-answers-ready`, completar patch ownership/vigencia controlado al 31-12-2025, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
