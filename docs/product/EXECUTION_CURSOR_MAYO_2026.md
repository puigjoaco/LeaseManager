...
| Fuente exacta | `main` en `428e253e`, despues del merge confirmado de PR #996 `codex/stage6-mirror-ownership-gate`. |
| Brecha activa | Paquete tactico `codex/stage6-mirror-proof-run-evidence`: cerrar el bypass posterior por el cual `audit_annual_tax_mirror_proof` podia evaluar DB/comparacion/readiness sin recibir evidencia explicita de la corrida `run_annual_tax_controlled_mirror` que genero la capa anual. El proof debe aceptar un `mirror_run` redactado, exigir que corresponda a empresa/AC/AT/source, que haya generado DB con 12 `MonthlyTaxFact`, ownership completo y sin blockers, y bloquear la conclusion si esa evidencia falta o viene bloqueada. Este thread queda dedicado a reconstruir y revisar la contabilidad/renta de Inmobiliaria Puig SpA. AC2024/AT2025 se usa como calibracion contra archivos preparados/presentados por contadora y confirmados por SII. AC2025/AT2026 se produce con evidencia local, auditoria y revision responsable. |
...
| Estado al cerrar paquete | En curso: este paquete hace que `audit_annual_tax_mirror_proof` requiera evidencia explicita del `run_annual_tax_controlled_mirror` para declarar prueba arquitectonica. La salida agrega `mirror_run_summary` sanitizado y bloquea si falta evidencia, si corresponde a otra empresa/AC/AT/source, si no genero DB, si trae blockers o si el ownership snapshot del run no esta completo. Mantiene `responsible_review_gate` como boundary de revision responsable y no lee `.env`, DB real, SII real, banco, EDIG ejecutable, correos ni integraciones externas. |
...
| Siguiente accion | Cerrar `codex/stage6-mirror-proof-run-evidence` con validaciones proporcionales, PR/CI/merge/limpieza. Si `main` ya contiene el merge y este worktree no existe, tratarlo como cerrado y continuar con el siguiente frente seguro desde este cursor. No crear PRs para reexplicar el goal ni para repetir paquetes #972-#996. |
