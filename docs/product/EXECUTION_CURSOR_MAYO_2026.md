...
| Fuente exacta | `main` en `59b7f0d0`, despues del merge confirmado de PR #948 `codex/stage6-ownership-handoff-consistency`. |
...
| Worktree | Paquete tactico actual: `D:/Proyectos/LeaseManager-stage6-apply-path-redaction` en rama `codex/stage6-apply-path-redaction`, solo mientras no este mergeado. Existe worktree historico pausado `C:/Users/puigj/.codex/worktrees/b2d9/LeaseManager` en rama `codex/thread-019ea306-rescue`; no tocar, no stagear y no subir sus PDFs/artefactos salvo decision explicita. |
| Rama | `codex/stage6-apply-path-redaction` para redactar errores de ruta del comando `apply_annual_tax_controlled_db_load`. |
| Estado | PR #948 ya esta mergeado. AC2024/AT2025 queda confirmado como prueba espejo `resuelto_confirmado` en evidencia local del 2026-06-17: carga controlada, ownership, bienes raices, DDJJ/F22, comparacion y Stage 6 sin blockers, como calibracion de arquitectura contra contadora/SII. AC2025/AT2026 tiene RCV/F29/libros/insumos anuales, checklist ownership con indice visual, respuestas responsables, workbench privado conectado y consistencia ownership handoff/writer protegida, pero sigue sin snapshot ownership controlado ni writer anual listo. Este paquete protege el CLI del writer para que errores de lectura/escritura no impriman rutas privadas con nombres o RUTs. |
...
| Estado al cerrar paquete | El cierre valido de este paquete debe dejar `apply_annual_tax_controlled_db_load` sin eco de rutas locales cuando falta el package, falla lectura o falla escritura del output; la salida versionable sigue restringida a `local-evidence/`. |
...
| Siguiente accion | Cerrar `codex/stage6-apply-path-redaction` con PR/CI/merge/limpieza. Luego completar el template de respuestas responsables mediante revision privada, validarlo con `materialize_company_accounting_responsible_answers`, materializar el workbench con ese review, completar patch ownership/vigencia controlado, validarlo, inyectarlo al paquete, seguir con paquete normalizado de carga DB local, writer controlado, espejo anual y comparacion revisable. |
