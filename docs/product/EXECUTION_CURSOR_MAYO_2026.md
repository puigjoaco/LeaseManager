...
| Campo | Valor |
| --- | --- |
| Frente activo | Sin paquete tactico abierto; ultimo cierre Etapa 1 - Perfil documental de arrendatario persona natural. |
| Fuente exacta | PR #185 `Require natural tenant document profile`, merge `d42d76b`, desde `01_Set_Vigente/PRD_CANONICO.md` linea 398. |
| Brecha activa | Cerrada localmente: `PoliticaFirmaYNotaria` puede exigir nacionalidad, estado civil y profesion para contrato principal; `Arrendatario`, API, snapshot, backoffice y auditor Etapa 1 quedan cubiertos. |
...
| Estado | PR #185 integrado en `main`, CI `acceptance` verde, worktree/rama tactica eliminados. |
