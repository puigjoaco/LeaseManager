...
| Estado | Auditor empresa/ano disponible por comando y expuesto en Reporting como `contabilidad/progreso-empresa/`; candidatos disponibles en `contabilidad/candidatos-progreso-empresa/`. Para Inmobiliaria Puig AC2024/AT2025 se agrego manifiesto read-only, plan de carga controlada, template de paquete normalizado, auditor de paquete, draft de valores, writer DB local `apply_annual_tax_controlled_db_load` y evidencia SQLite local ignorada. Balance General, RLI/CPT/RAI, DDJJ y F22 quedan como comparacion, no como calculo. El template real confirma refs mensuales completas, libros anuales presentes y F29 febrero/diciembre `no_aplica`; el draft real extrae Libro Diario, Libro Mayor, F29 y remuneraciones desde fuentes permitidas, rellena 176 campos, queda `ready_for_db_writer=true`, `missing_paths_count=0`, `invalid_paths_count=0`, y el writer materializa 12 cierres, 12 LibroDiario, 12 LibroMayor, 12 BalanceComprobacion, 10 obligaciones, 10 F29 y 12 MonthlyTaxFact normalizados en DB local. Gate Etapa 6 sobre esa DB queda `classification=parcial`, `ready_for_stage6_renta_anual=false` por faltantes anuales: capacidades DDJJ/F22, source bundle en DB, proceso anual, DDJJ/F22, documento soporte y comparador. |
...
| Siguiente accion | Validar suite impactada del paquete `codex/ac2024-mirror-source-bundle`, cerrar PR/CI/merge cuando sea posible, y continuar con la capa anual de prueba espejo: crear capacidades controladas DDJJ/F22, persistir `AnnualTaxSourceBundle`, generar `ProcesoRentaAnual`/DDJJ/F22/dossier local desde los 12 `MonthlyTaxFact`, y luego implementar comparador contra outputs esperados sin usarlos como input. |
