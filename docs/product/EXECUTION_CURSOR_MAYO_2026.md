...
| Estado | Auditor empresa/ano disponible por comando y expuesto en Reporting como `contabilidad/progreso-empresa/`; candidatos disponibles en `contabilidad/candidatos-progreso-empresa/`. Para Inmobiliaria Puig AC2024/AT2025 se agrego manifiesto read-only, plan de carga controlada, template de paquete normalizado y writer DB local `apply_annual_tax_controlled_db_load`. Balance General, RLI/CPT/RAI, DDJJ y F22 quedan como comparacion, no como calculo. El plan real queda `ready_for_db_load=false` hasta completar valores normalizados desde libros anuales, F29 PDF y remuneraciones, mas comparador contra outputs esperados. |
