...
| Operacion | PRD, ADR identidad envio | `backend/operacion`, backoffice operacion | 1 | implementado_sin_evidencia | Cuentas, mandatos e identidades validadas | Cuentas y recaudador soportan empresa, comunidad o socio; usar `audit_stage1_matrix` contra snapshot/DB autorizada para cerrar matriz entidad-cuenta-mandato. |
| Contratos | PRD, reglas contractuales | `backend/contratos`, backoffice contratos | 1 | implementado_sin_evidencia | Matriz contrato-propiedad-periodo-garantia | Usar `audit_stage1_matrix`; calendario mensual, continuidad de periodos y minimo operativo ya tienen gate local. |
