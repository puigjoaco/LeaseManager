...
Nota 2026-06-22: Etapa 6/Renta Anual aplica el boundary controlado al mirror
anual directo. `run_annual_tax_controlled_mirror` rechaza `source_label`,
`authorization_ref`, `responsible_ref`, `fiscal_rule_ref` y
`certificates_proof_ref` con RUT chileno o rutas locales absolutas antes de
declarar readiness, abrir transaccion o crear `AnnualTaxSourceBundle`,
...
rechaza `company_ref`, `responsible_ref`, `approval_ref` y
`candidate_sources[].path_ref` con RUT chileno o rutas locales absolutas antes
...
Nota 2026-06-22: Etapa 6/Renta Anual centraliza la validacion de salidas locales
controladas en `core.management.local_evidence_paths`. Los comandos
`audit_annual_tax_controlled_package_readiness`,
`apply_annual_tax_controlled_db_load`, `run_annual_tax_controlled_mirror` y
`audit_annual_tax_mirror_proof` comparten la misma regla: si `--output` queda
dentro del repo debe estar bajo `local-evidence/`, y su ruta relativa no puede
contener RUT chileno, rutas locales absolutas ni refs sensibles antes de
...
Nota 2026-06-22: Etapa 6/Renta Anual centraliza el detector de RUT chileno en
`core.reference_validation`. Manifest, handoffs, readiness, preguntas,
respuestas, intake documental, cobertura bancaria y review ownership dejan de
...
Nota 2026-06-22: Etapa 6/Renta Anual alinea
`build_annual_tax_controlled_values_draft` con esos detectores compartidos. El
draft controlado rechaza refs responsables/aprobacion contaminadas, vacia
`source_root_ref` sensible y limpia `path_ref` embebidos en package,
input_source_refs, targets de comparacion, F29, payroll, ledger, labor
...
DDJJ/F22 y `manifest.json` verificable por hash/tamano.
`verify_annual_tax_export_file_package` reabre el paquete desde disco y rechaza
manifest o archivos desalineados, archivos extra, tamper, JSON no canonico o
ruptura del boundary oficial. El paquete sigue siendo local/controlado: no
...
WebPay bloqueados o fallidos. `IntentoPagoWebPay.clean()` rechaza estados
`bloqueado_gate`/`fallido` sin `motivo_bloqueo` no vacio y no sensible, y
`audit_stage2_cobranza_readiness` reporta
`stage2.webpay_intent.block_reason_missing` para snapshots heredados con
bloqueos o fallos sin razon operativa.

Nota 2026-06-17: Cobranza/WebPay deja auditables los intentos fallidos.
`fail_prepared_webpay_intent()` marca un intento `preparado` como `fallido`
solo con actor y motivo operativo, crea resolucion manual local y emite
`cobranza.webpay_intento.failed` en la misma transaccion. La readiness reporta
...
Nota 2026-06-21: El handoff responsable conserva esos `safe_issue_codes` al
pasar por review de respuestas, paquete verificable y preflight. El normalizador
de `question_source_summaries` redacta codigos o severidades contaminadas con
...
Nota 2026-06-22: El redactor controlado de payloads SII conserva valores
vacios como ausencia de dato y no los convierte en marcadores de redaccion. Los
valores no vacios bajo claves sensibles, con RUT chileno, ruta local absoluta,
