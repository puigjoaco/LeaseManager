...
| Patrimonio | PRD, modelo canonico | `backend/patrimonio`, backoffice patrimonio | 1 | implementado_sin_evidencia | Datos reales/snapshot y validacion de entidades | Auditor Etapa 1 valida socios, empresas, comunidades, participaciones, representaciones y propiedades; ejecutar `scripts/run-stage1-snapshot-gate.ps1` contra snapshot/DB autorizada. |
| Operacion | PRD, ADR identidad envio | `backend/operacion`, backoffice operacion | 1 | implementado_sin_evidencia | Cuentas, mandatos e identidades validadas | Cuentas/recaudador soportan empresa, comunidad o socio; entidad facturadora exige `ConfiguracionFiscalEmpresa` activa con regimen tributario activo; usar `scripts/run-stage1-snapshot-gate.ps1` contra snapshot/DB autorizada. |
| Contratos | PRD, reglas contractuales | `backend/contratos`, backoffice contratos | 1 | implementado_sin_evidencia | Matriz contrato-propiedad-periodo-garantia | Usar `scripts/run-stage1-snapshot-gate.ps1`; arrendatarios, contacto/domicilio operativo, representante legal de arrendatario empresa, contratos, periodos, calendario mensual, continuidad de periodos, minimo operativo, propiedad vinculada sin contrato independiente, contrato futuro con aviso/terminacion, codigo efectivo `001-999` por cuenta y coherencia de garantias ya tienen gate local. |
