...
| Operacion | PRD, ADR identidad envio | `backend/operacion`, backoffice operacion | 1 | implementado_sin_evidencia | Cuentas, mandatos e identidades validadas | Cuentas/recaudador soportan empresa, comunidad o socio; entidad facturadora exige `ConfiguracionFiscalEmpresa` activa; usar `audit_stage1_matrix` contra snapshot/DB autorizada. |
| Contratos | PRD, reglas contractuales | `backend/contratos`, backoffice contratos | 1 | implementado_sin_evidencia | Matriz contrato-propiedad-periodo-garantia | Usar `audit_stage1_matrix`; arrendatarios, contratos, periodos, calendario mensual, continuidad de periodos, minimo operativo, propiedad vinculada sin contrato independiente, codigo efectivo por cuenta y coherencia de garantias ya tienen gate local. |
