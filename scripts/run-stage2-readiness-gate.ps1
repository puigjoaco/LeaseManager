param(
    ...
    [string]$SourceLabel = 'stage2-local-readiness',

    [string]$AuthorizationRef = '',

    [string]$Stage1EvidenceRef = '',

    [string]$EmailProofRef = '',

    [string]$WebPayProofRef = '',
...
if ($usesDefaultDatabase) {
    $DatabaseUrl = "sqlite:///local-evidence/stage2/readiness/stage2_empty_$timestamp.sqlite3"
}

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repoRoot "local-evidence\stage2\readiness\stage2_readiness_$timestamp.json"
}
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition (Test-NonSensitiveReference $Stage1EvidenceRef) 'Stage1EvidenceRef es obligatorio para cierre Etapa 2 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $EmailProofRef) 'EmailProofRef es obligatorio para cierre Etapa 2 y debe ser no sensible.'
...
Write-Host "Stage 2 readiness gate" -ForegroundColor Cyan
...
try {
    ...
    if ($shouldRunMigrations) {
        ...
        Assert-Condition ($LASTEXITCODE -eq 0) 'manage.py migrate fallo para readiness Etapa 2.'
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($Stage1EvidenceRef)) {
        $auditArgs += @('--stage1-evidence-ref', $Stage1EvidenceRef.Trim())
    }
    if (-not [string]::IsNullOrWhiteSpace($EmailProofRef)) {
        $auditArgs += @('--email-proof-ref', $EmailProofRef.Trim())
    }
    if (-not [string]::IsNullOrWhiteSpace($WebPayProofRef)) {
        $auditArgs += @('--webpay-proof-ref', $WebPayProofRef.Trim())
    }
    ...
    & $pythonExe @auditArgs
    Assert-Condition ($LASTEXITCODE -eq 0) 'audit_stage2_cobranza_readiness fallo.'
}
finally {
    Pop-Location
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition ($audit.ready_for_stage2_cobranza -eq $false) 'La fuente local/fixture/demo no puede cerrar Etapa 2.'
    Assert-Condition ($issueCodes -contains 'stage2.source_kind_not_authorized') 'La auditoria local debe reportar stage2.source_kind_not_authorized.'
}
if ($RequireReady) {
    Assert-Condition ($audit.ready_for_stage2_cobranza -eq $true) 'RequireReady exige ready_for_stage2_cobranza=true.'
    Assert-Condition ($audit.classification -eq 'resuelto_confirmado') 'RequireReady exige classification=resuelto_confirmado.'
}

Write-Host "Stage 2 readiness gate OK: classification=$($audit.classification), ready_for_stage2_cobranza=$($audit.ready_for_stage2_cobranza)" -ForegroundColor Green
Write-Host "Output: $resolvedOutput"
