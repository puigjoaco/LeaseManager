...
if (-not $OnlySmoke) {
    ...
    try {
        ...
        [ordered]@{
            ...
            source_kind = 'backup_autorizado'
            authorization_ref = 'restore-authorization-v1'
            backupPath = 'D:\private\backups\lease-manager-prod.dump'
        ...
        $stage7RawBackupFileOutput = & $stage7ReadinessScript `
            ...
            -FinalAcceptanceEvidencePath $finalAcceptanceAuthorizedPath `
            -SkipMigrations | Out-String
        Assert-Condition ($LASTEXITCODE -eq 0) 'run-stage7-readiness-gate fallo al validar restore con alias crudo de backup.'
        ...
        Assert-Condition ($stage7RawBackupFileIssueCodes -contains 'stage7.restore_backup_file_not_allowed') 'Restore con backupPath crudo debe tener codigo especifico.'
        Assert-Condition ($stage7RawBackupFileReadiness.restore_evidence.has_backup_file -eq $true) 'Restore con backupPath crudo debe marcar has_backup_file=true.'
        Assert-Condition ($stage7RawBackupFileReadiness.restore_evidence.has_backup_ref -eq $false) 'Restore con solo backupPath no debe marcar has_backup_ref=true.'
        Assert-Condition ($stage7RawBackupFileReadiness.checks.restore_authorized_evidence -eq $false) 'Restore con backupPath crudo no debe quedar autorizado para cierre.'
