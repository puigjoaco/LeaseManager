param(
    ...
    [string]$SourceLabel = 'stage5-local-readiness',

    [string]$AuthorizationRef = '',

    [string]$Stage3EvidenceRef = '',

    [string]$LedgerProofRef = '',

    [string]$ReportsProofRef = '',
...
if ($usesDefaultDatabase) {
    $DatabaseUrl = "sqlite:///local-evidence/stage5/readiness/stage5_empty_$timestamp.sqlite3"
}

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repoRoot "local-evidence\stage5\readiness\stage5_readiness_$timestamp.json"
}
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition (Test-NonSensitiveReference $Stage3EvidenceRef) 'Stage3EvidenceRef es obligatorio para cierre Etapa 5 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $LedgerProofRef) 'LedgerProofRef es obligatorio para cierre Etapa 5 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $ReportsProofRef) 'ReportsProofRef es obligatorio para cierre Etapa 5 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $ResponsibleRef) 'ResponsibleRef es obligatorio para cierre Etapa 5 y debe ser no sensible.'
}
...
Write-Host "Stage 5 readiness gate" -ForegroundColor Cyan
...
try {
    ...
    if ($shouldRunMigrations) {
        ...
        Assert-Condition ($LASTEXITCODE -eq 0) 'manage.py migrate fallo para readiness Etapa 5.'
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($Stage3EvidenceRef)) {
        $auditArgs += @('--stage3-evidence-ref', $Stage3EvidenceRef.Trim())
    }
    if (-not [string]::IsNullOrWhiteSpace($LedgerProofRef)) {
        $auditArgs += @('--ledger-proof-ref', $LedgerProofRef.Trim())
    }
    if (-not [string]::IsNullOrWhiteSpace($ReportsProofRef)) {
        $auditArgs += @('--reports-proof-ref', $ReportsProofRef.Trim())
    }
    ...
    & $pythonExe @auditArgs
    Assert-Condition ($LASTEXITCODE -eq 0) 'audit_stage5_contabilidad_readiness fallo.'
}
finally {
    Pop-Location
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition ($audit.ready_for_stage5_contabilidad -eq $false) 'La fuente local/fixture/demo no puede cerrar Etapa 5.'
    Assert-Condition ($issueCodes -contains 'stage5.source_kind_not_authorized') 'La auditoria local debe reportar stage5.source_kind_not_authorized.'
}
if ($RequireReady) {
    Assert-Condition ($audit.ready_for_stage5_contabilidad -eq $true) 'RequireReady exige ready_for_stage5_contabilidad=true.'
    Assert-Condition ($audit.classification -eq 'resuelto_confirmado') 'RequireReady exige classification=resuelto_confirmado.'
}

Write-Host "Stage 5 readiness gate OK: classification=$($audit.classification), ready_for_stage5_contabilidad=$($audit.ready_for_stage5_contabilidad)" -ForegroundColor Green
Write-Host "Output: $resolvedOutput"
