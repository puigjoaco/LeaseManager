param(
    ...
    [string]$SourceLabel = 'stage6-local-readiness',
...
if ($usesDefaultDatabase) {
    $DatabaseUrl = "sqlite:///local-evidence/stage6/readiness/stage6_empty_$timestamp.sqlite3"
}

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repoRoot "local-evidence\stage6\readiness\stage6_readiness_$timestamp.json"
}
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition (Test-NonSensitiveReference $Stage5EvidenceRef) 'Stage5EvidenceRef es obligatorio para cierre Etapa 6 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $Stage4SiiEvidenceRef) 'Stage4SiiEvidenceRef es obligatorio para cierre Etapa 6 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $FiscalRuleRef) 'FiscalRuleRef es obligatorio para cierre Etapa 6 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $CertificatesProofRef) 'CertificatesProofRef es obligatorio para cierre Etapa 6 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $ResponsibleRef) 'ResponsibleRef es obligatorio para cierre Etapa 6 y debe ser no sensible.'
}
...
Write-Host "Stage 6 readiness gate" -ForegroundColor Cyan
...
try {
    ...
    if ($shouldRunMigrations) {
        ...
        Assert-Condition ($LASTEXITCODE -eq 0) 'manage.py migrate fallo para readiness Etapa 6.'
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($Stage5EvidenceRef)) {
        $auditArgs += @('--stage5-evidence-ref', $Stage5EvidenceRef.Trim())
    }
    if (-not [string]::IsNullOrWhiteSpace($Stage4SiiEvidenceRef)) {
        $auditArgs += @('--stage4-sii-evidence-ref', $Stage4SiiEvidenceRef.Trim())
    }
    if (-not [string]::IsNullOrWhiteSpace($FiscalRuleRef)) {
        $auditArgs += @('--fiscal-rule-ref', $FiscalRuleRef.Trim())
    }
    if (-not [string]::IsNullOrWhiteSpace($CertificatesProofRef)) {
        $auditArgs += @('--certificates-proof-ref', $CertificatesProofRef.Trim())
    }
    ...
    & $pythonExe @auditArgs
    Assert-Condition ($LASTEXITCODE -eq 0) 'audit_stage6_renta_anual_readiness fallo.'
}
finally {
    Pop-Location
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition ($audit.sections.final_evidence.stage5_evidence_ref -eq $true) 'La evidencia Etapa 5 debe ser trazable.'
    Assert-Condition ($audit.sections.final_evidence.stage4_sii_evidence_ref -eq $true) 'La evidencia SII anual debe ser trazable.'
    Assert-Condition ($audit.sections.final_evidence.fiscal_rule_ref -eq $true) 'La referencia de regla fiscal debe ser trazable.'
    Assert-Condition ($audit.sections.final_evidence.certificates_proof_ref -eq $true) 'La referencia de certificados debe ser trazable.'
    ...
    Assert-Condition ($audit.ready_for_stage6_renta_anual -eq $false) 'La fuente local/fixture/demo no puede cerrar Etapa 6.'
    Assert-Condition ($issueCodes -contains 'stage6.source_kind_not_authorized') 'La auditoria local debe reportar stage6.source_kind_not_authorized.'
}
if ($RequireReady) {
    Assert-Condition ($audit.ready_for_stage6_renta_anual -eq $true) 'RequireReady exige ready_for_stage6_renta_anual=true.'
    Assert-Condition ($audit.classification -eq 'resuelto_confirmado') 'RequireReady exige classification=resuelto_confirmado.'
}

Write-Host "Stage 6 readiness gate OK: classification=$($audit.classification), ready_for_stage6_renta_anual=$($audit.ready_for_stage6_renta_anual)" -ForegroundColor Green
Write-Host "Output: $resolvedOutput"
