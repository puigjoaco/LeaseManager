param(
    ...
    [string]$SourceLabel = 'stage4-local-readiness',

    [string]$AuthorizationRef = '',

    [string]$Stage5EvidenceRef = '',

    [string]$EnvironmentProofRef = '',
...
if ($usesDefaultDatabase) {
    $DatabaseUrl = "sqlite:///local-evidence/stage4/readiness/stage4_empty_$timestamp.sqlite3"
}

if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repoRoot "local-evidence\stage4\readiness\stage4_readiness_$timestamp.json"
}
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition (Test-NonSensitiveReference $Stage5EvidenceRef) 'Stage5EvidenceRef es obligatorio para cierre Etapa 4 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $EnvironmentProofRef) 'EnvironmentProofRef es obligatorio para cierre Etapa 4 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $FiscalRuleRef) 'FiscalRuleRef es obligatorio para cierre Etapa 4 y debe ser no sensible.'
    Assert-Condition (Test-NonSensitiveReference $ResponsibleRef) 'ResponsibleRef es obligatorio para cierre Etapa 4 y debe ser no sensible.'
}
...
Write-Host "Stage 4 readiness gate" -ForegroundColor Cyan
...
try {
    ...
    if ($shouldRunMigrations) {
        ...
        Assert-Condition ($LASTEXITCODE -eq 0) 'manage.py migrate fallo para readiness Etapa 4.'
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($EnvironmentProofRef)) {
        $auditArgs += @('--environment-proof-ref', $EnvironmentProofRef.Trim())
    }
    ...
    & $pythonExe @auditArgs
    Assert-Condition ($LASTEXITCODE -eq 0) 'audit_stage4_sii_readiness fallo.'
}
finally {
    Pop-Location
...
if ($isAuthorizedSourceKind) {
    ...
    Assert-Condition ($audit.sections.final_evidence.environment_proof_ref -eq $true) 'La prueba de ambiente SII debe ser trazable.'
    ...
    Assert-Condition ($audit.sections.final_evidence_sensitive.environment_proof_ref -eq $false) 'La prueba de ambiente SII no debe ser sensible.'
    ...
    Assert-Condition ($audit.ready_for_stage4_sii -eq $false) 'La fuente local/fixture/demo no puede cerrar Etapa 4.'
    Assert-Condition ($issueCodes -contains 'stage4.source_kind_not_authorized') 'La auditoria local debe reportar stage4.source_kind_not_authorized.'
}
if ($RequireReady) {
    Assert-Condition ($audit.ready_for_stage4_sii -eq $true) 'RequireReady exige ready_for_stage4_sii=true.'
    Assert-Condition ($audit.classification -eq 'resuelto_confirmado') 'RequireReady exige classification=resuelto_confirmado.'
}

Write-Host "Stage 4 readiness gate OK: classification=$($audit.classification), ready_for_stage4_sii=$($audit.ready_for_stage4_sii)" -ForegroundColor Green
Write-Host "Output: $resolvedOutput"
