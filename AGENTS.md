...
`git status --short --branch` y `git worktree list`. La bitacora operativa no
reemplaza al PRD ni a las fuentes rectoras: solo fija el frente activo para evitar que
...
archivo de continuidad o descartarse con instruccion segura antes de abrir otro frente de
