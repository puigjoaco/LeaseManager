...
`Contabilidad` / `SII` / `Reporting` ya tienen una primera base mensual no vacia para empresa `1`, con `DTE` borrador y `F29` preparada en `2026-05`, y un primer bloque anual en `preparado` para `2027`, pero siguen lejos de un dataset rico.

El proyecto podria seguir por una de estas dos lineas:

- profundizar la utilidad de `Contabilidad` / `SII` / `Reporting` ahora que el entorno remoto ya tiene base estructural y una primera actividad mensual;
...
- si el MCP de browser vuelve a fallar, usar `scripts/smoke-public-backoffice.mjs` como ruta estable de smoke visual;
